...
2. Sube los secretos al proyecto Firebase (por defecto `encuestas-prometheus-9tzwei`):
   - **Windows (`.env` en la raíz):** `pwsh ./tool/deploy_stripe_secrets.ps1`
   - **Windows (otro archivo):** `pwsh ./tool/deploy_stripe_secrets.ps1 -EnvPath C:\ruta\claves.env`
   - **Windows (sin archivo, solo esta sesión):** define `$env:STRIPE_SECRET_KEY_LIVE` y `$env:STRIPE_SECRET_KEY_TEST` y ejecuta `pwsh ./tool/deploy_stripe_secrets.ps1 -FromEnvironment`
...
2. **Google Cloud SDK** (`gcloud` / `gsutil`): si PowerShell dice que **no reconoce `gcloud`**, el SDK no está en el `PATH`. Desde la raíz del repo ejecuta **`. .\tool\cloud_sdk_path.ps1`** (con el punto inicial) y en esa misma ventana: `gcloud auth login` y `gcloud config set project encuestas-prometheus-9tzwei`. Opción permanente: Variables de entorno del usuario → `Path` → añadir `...\Google\Cloud SDK\google-cloud-sdk\bin`.
3. Aplicar CORS:
