...
String merchantIdentifier() =>
    isAndroid ? '' : RuntimeEnv.stripeAppleMerchantId;

Future initializeStripe() async {
  RuntimeEnv.debugLogMissingStripe();
  if (!RuntimeEnv.hasStripePublishableKey) {
    if (kDebugMode) {
      debugPrint('[Stripe] Inicialización omitida: falta STRIPE_PUBLISHABLE_KEY.');
    }
    return;
  }
  Stripe.publishableKey = RuntimeEnv.stripePublishableKey;
  final mid = merchantIdentifier();
  if (mid.isNotEmpty) {
  ...
  try {
    if (!RuntimeEnv.hasStripePublishableKey) {
      return const StripePaymentResponse(
        errorMessage:
            'Pagos no configurados. Define STRIPE_PUBLISHABLE_KEY (ver README).',
      );
    }
    final callName = RuntimeEnv.stripeUseLiveBackend
        ? 'initStripePayment'
    ...
    await Stripe.instance.initPaymentSheet(
      paymentSheetParameters: SetupPaymentSheetParameters(
        ...
        merchantDisplayName: 'Prometheus',
        googlePay: allowGooglePay
            ? PaymentSheetGooglePay(
                merchantCountryCode: 'MX',
                currencyCode: currency,
                testEnv: !RuntimeEnv.stripeUseLiveBackend,
              )
            : null,
