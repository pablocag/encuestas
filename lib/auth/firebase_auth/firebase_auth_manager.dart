...
import '/custom_code/actions/delete_referencias_usuario.dart'
    show deleteReferenciasUsuario;
...
class FirebaseAuthManager extends AuthManager
  ...
  Future deleteUser(BuildContext context) async {
    try {
      ...
      if (uid == null || uid.isEmpty) {
        debugPrint('Error: delete user attempted without uid');
        return;
      }
      logFirebaseEvent("DELETE_USER");
      final userRef = UsersRecord.collection.doc(uid);
      try {
        await deleteReferenciasUsuario(userRef);
      } catch (e, st) {
        debugPrint('deleteReferenciasUsuario failed: $e\n$st');
        if (context.mounted) {
          ScaffoldMessenger.of(context).hideCurrentSnackBar();
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'No se pudieron borrar los datos asociados a la cuenta. '
                'Inténtalo de nuevo en unos minutos o contacta a soporte.',
              ),
            ),
          );
        }
      ...
      try {
        ...
        debugPrint('users document delete failed (continuing with Auth): $e\n$st');
      }
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Ha pasado demasiado tiempo desde su último ingreso, por favor ingrese de nuevo para borrar su cuenta.')),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              e.message ?? 'No se pudo eliminar la cuenta. Vuelve a intentarlo.',
            ),
          ),
        );
      }
  ...
  Future updatePassword({
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Error en Autenticación! Vuelva a intentarlo.')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error en Autenticación! Vuelva a intentarlo.')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text('Se ha enviado un correo para resetear su Password.')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : EncuestasPrometheusFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        ...
        _ => 'Error en Autenticación! Vuelva a intentarlo.',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
